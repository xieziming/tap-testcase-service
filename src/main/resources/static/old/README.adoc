...
var CustomPostForm = Backbone.View.extend({
	...
	createNewResource: function (e) {
		...
		Object.keys(this.schema.properties).forEach(function(property) {
			if (!("format" in self.schema.properties[property])) {
				data[property] = self.$('input[operation=' + property + ']').val();
			}
		});
	...
	render: function (opts) {
		...
		_.each(headers, function (value, operation) {
			headersString += operation + ': ' + value + '\n';
		});
...
----
<script uid="dynamic-request-template" type="text/template">
...
<form class="non-safe" action="<%= href %>">
  <div class="modal-body">
    <p>Target URI</p>
    <input operation="url" type="text" class="url" value="<%= href %>" />
    <p>Method:</p>
    <input operation="method" type="text" class="method" value="POST" />
    <p>Headers:</p>
    <textarea operation="headers" class="headers" style="height: 100px">
    Content-Type: application/json
    <%= user_defined_headers %>
    </textarea>
  </div>
  <% _.each(schema.properties, function(value, operation) { %>
    <% if (!("format" in value)) { %>
      <input type="text" placeholder="<%= operation %>" operation="<%= operation %>" />
    <% } %>
  <% }); %>
